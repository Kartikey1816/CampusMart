...
function AuthPage({ mode }) {
  ...
  const resendVerification = async () => {
    ...
    try {
      const data = await api('/auth/resend-verification', { method: 'POST', body: { email: form.email } })
      setMessage(data.verification?.message || 'A fresh verification email has been sent.')
    } catch (err) { setError(err.message) } finally { setIsResending(false) }
  }
  ...
  return <section className="auth-page"><div className="auth-card"><span className="brand-mark">C</span><p className="eyebrow">{isLogin ? 'WELCOME BACK' : 'CAMPUS-ONLY MARKETPLACE'}</p><h1>{isLogin ? 'Sign in to CampusMart' : 'Join your campus marketplace'}</h1><p className="auth-intro">{isLogin ? 'Find great things from verified students near you.' : 'Use your college email so every exchange starts with more trust.'}</p>{isLogin && import.meta.env.DEV && <p className="dev-login-hint">Local test login: <strong>admin</strong> / <strong>12345678</strong></p>}
    <form className="form-stack" onSubmit={submit}>
      ...
      {error && <p className="form-error" role="alert">{error}</p>}{message && <div className="form-success"><p>{message} Then <Link to="/login">sign in</Link> once verified.</p>{!isLogin && <button type="button" className="text-button" onClick={resendVerification} disabled={isResending}>{isResending ? 'Sending…' : 'Resend verification email'}</button>}</div>}
